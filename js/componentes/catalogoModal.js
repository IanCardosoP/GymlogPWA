...
export function abrirCatalogoModal({ onSeleccionar, onCrearPersonalizado, onCerrar }) {
  ...
  // Segunda capa: el ejercicio en grande + instrucciones, para confirmar antes de
  // comprometerlo a la rutina. Volver deja el listado intacto (búsqueda, filtros
  // y scroll siguen como estaban).
  const abrirPreview = (entrada) => {
    abrirPreviewEjercicio({
      candidatos: [entrada], // una sola: aquí el usuario ya eligió cuál quiere ver
      etiquetaConfirmar: '[ + AGREGAR ]',
      onConfirmar: (elegida) => {
        cerrar(); // cierra el catálogo también
